...
impl Render {
	pub fn new() -> Result<Render, String> {
		let extensions = InstanceExtensions {
			khr_surface: true,
			//khr_display: true, // not present
			//khr_display_swapchain: true, // not present
		...
		let (swapchain, images) = {
			let caps = surface.get_capabilities(&physical).map_err(|e| format!("Failed to get surface capabilities: {}", e))?;
			
			let dimensions = caps.current_extent.unwrap_or([600, 800]);
			let present = caps.present_modes[0];
			let usage = caps.supported_usage_flags;
			
			Swapchain::new(&device, &surface, /*num_images*/3,
		...
		let vertex_buffer: Arc<Buffer<[Vertex], _>> =  Buffer::array(&device, 3, &buffer::Usage::all(), HostVisible, &queue)
		...
		let vs = shaders::TestVert::load(&device).map_err(|e| format!("Failed to load shader: {}", e))?;
		let fs = shaders::TestFrag::load(&device).map_err(|e| format!("Failed to load shader: {}", e))?;
		
		// Create command buffer pool
		let cb_pool = CommandBufferPool::new(&device, &queue.family()).map_err(|e| format!("Failed to create command buffer pool: {}", e))?;
		...
		let renderpass = renderpass::CustomRenderPass::new(&device).map_err(|e| format!("Failed to create renderpass: {}", e))?;
		...
		let pipeline = {
			...
			pipeline::GraphicsPipeline::new(&device, SingleBufferDefinition::<Vertex>::new(),
